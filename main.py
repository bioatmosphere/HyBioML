...
site = 'US-Ho1' #US-SPR; US-Ho1; US-MOz; US-MoT
pft = int(0)
lb = int(0)
rb = int(365)
...
model.run_selm(use_nn=True, use_MPI=False, prefix='3R_default', seasonal_rootalloc=False, spinup_cycles=2)
...
#create model object
model2 = models.MyModel(site=site,nfroot_orders=1)
#Load model forcings
model2.load_forcings(site=site)
print('running model with fine root phenology')
#model2.parms['froot_phen_width'][:]=0.6
#model2.parms['froot_phen_peak'][:]=0.5
model2.run_selm(use_nn=True, prefix='1R_default', seasonal_rootalloc=False, spinup_cycles=2)
